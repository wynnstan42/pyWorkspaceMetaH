...
'''
    調參方法: 
    Step 1: 將固定 random seed 的程式碼註解掉，以增加隨機性
        >> 結果: 此時出現 31, 32, 42... 不等的結果
    Step 2: 調低一點波茲曼常數 K，以求增加轉移狀態的機率 (K=0.7)
        >> 結果: 此時較常出現低於 40 的得分
    Step 4: 希望將遞迴次數增加，故調高最大遞迴次數以及最高溫度 (MAXIT=6000, T_high=6000)
        >> 結果: 此時較常出現 30, 31，偶爾仍會出現 35, 38 甚至 42
    Step 5: 發現遞迴次數增加並未符合預期，猜測是溫度下降速度過快，故調整降溫比率，並將熱平衡迴圈數增加 (T_scale=0.98, DWELL=30)
        >> 結果: 此時較常取得得分 30，偶爾出現 31, 32
    
    >> 最終結果: 最佳解 [1 0 6 7 8 5 4 3 2]，得分 30
'''

# Step 0: 設定各個參數值
MAXIT = 6000    # maximal iteration number -> 主要最大迴圈數
K = 0.7    # Boltzmann rate -> 影響在新的解得分未較好時，仍然可轉移狀態的機率
DWELL = 30  # 計算平衡狀態時需要的迴圈數目 -> 每階段的熱平衡迴圈數
T_high = 6000.0  # 初始溫度 -> 較高的溫度會有較高的轉移機率
T_scale = 0.98  # 每階段降溫比率: t0 >> t0*r >> t0*r*r >> ... -> 介於 0~1 (!=0)，越低則降溫愈快
