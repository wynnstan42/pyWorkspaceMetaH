...
'''
    Homogeneous:
    >> Annealing with constant temperature (prob.) for each loops
    >> two loops (inner/outer)
    >> T keep constant in inner loop and decrease in outer loop
    
    Inhomogeneous:
    >> Annealing with decrease temperature (prob.)
    >> Only **one** loop
    >> T decrease for each loop
        >> in the beginning nearly all pertubations are accepted (expploration)
        >> at the end T is small so only small number of pertubations are accepted (exploitation)
'''

# Step 0: 設定各個參數值
MAXIT = 1000  # maximal iteration number -> 主要最大迴圈數
K = 0.9  # Boltzmann rate -> 影響在新的解得分未較好時，仍然可轉移狀態的機率
T_high = 1000.0  # 初始溫度 -> 較高的溫度會有較高的轉移機率
T_scale = 0.998  # 每階段降溫比率: t0 >> t0*r >> t0*r*r >> ... -> 介於 0~1 (!=0)，越低則降溫愈快
...
while num_it < MAXIT and t > T_low:     # 若達最大遞迴數或溫度已達最低溫度則終止迴圈計算
    # Step 5.1.1: 找鄰居 xnew -> 透過隨機替換兩城市排序位置取得鄰居解
    xnew = x.copy()
    ...
    t *= T_scale  # 繼續降溫 -> 降溫速度取決於一開始設定的每階段降溫比率
    num_it += 1  # 增加目前遞迴次數
